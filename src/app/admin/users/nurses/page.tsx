 ...
 const transformedNurses = useMemo(() => {
  ...
  return rawData.map(
   (nurse: Record<string, unknown>, index: number): Nurse => ({
    uid: (nurse.uid as string) || (nurse.id as string) || (nurse.nurseId as string) || `nurse-${index}`,
    id: (nurse.id as string) || (nurse.uid as string) || (nurse.nurseId as string) || `nurse-${index}`,
    ...
    specialization:
     (nurse.specialization as string) ||
     (nurse.specialty as string) ||
     (nurse.title as string) ||
     "",
    experience_yrs:
     (nurse.experience_yrs as string) ||
     (nurse.experience as string) ||
     (nurse.years_of_experience as string) ||
     (nurse.yearsOfExperience as string) ||
     "",
    ...
    address: (nurse.address as string) || (nurse.location as string) || "",
    ...
    nurseId: (nurse.nurseId as string) || (nurse.uid as string) || (nurse.id as string),
 ...
 const handleSaveNurse = async () => {
  ...
  try {
   ...
   await createFirebaseDocument("users", {
    ...
    experience: formData.experience_yrs,
           ...
           <td className="px-6 py-4 whitespace-nowrap">
            <div className="text-[10px] md:text-[12px] text-gray-900">
             {nurse.experience_yrs
              ? nurse.experience_yrs.toString().toLowerCase().includes("year")
               ? nurse.experience_yrs
