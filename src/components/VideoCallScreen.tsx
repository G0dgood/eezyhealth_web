...
const GlassCallControls: React.FC<{ onLeave: () => void }> = ({ onLeave }) => {
  const call = useCall();
  ...
  return (
    <div className="flex items-center gap-4 px-6 py-3 rounded-full backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl">
      <button
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 relative">
      ...
      <div className="absolute bottom-8 left-0 right-0 flex justify-center w-full z-10">
        <GlassCallControls onLeave={onLeave} />
      </div>
    </div>
  );
};
